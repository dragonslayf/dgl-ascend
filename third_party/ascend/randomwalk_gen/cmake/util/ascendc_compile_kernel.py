...
class CompileKernel:
    ...
    def ascendc_build(self: any):
        ...
        hi_python = os.getenv("ASCEND_PYTHON_EXECUTABLE", "python3")
        if os.getenv("TILINGKEY_PAR_COMPILE") is None:
            cmd_str = ('export HI_PYTHON="{}" && export ASCEND_CUSTOM_OPP_PATH={} && export TILINGKEY_PAR_COMPILE=1'
                       '&& make -f {} PY={} OUT={} CPP={}')
        else:
            cmd_str = ('export HI_PYTHON="{}" && export ASCEND_CUSTOM_OPP_PATH={} && make -f {} PY={} OUT={} CPP={}')

        if os.system(cmd_str.format(hi_python, self.build_opp_path, mkfile, self.op_impl_py, op_bin_dir, self.op_cpp_file)) != 0:
